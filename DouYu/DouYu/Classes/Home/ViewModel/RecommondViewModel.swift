...
class RecommondViewModel : BaseAnchorViewModel{
    // 懒加载存放所有模块的数组
...
//         print(NSDate.getCurrentTime())
        disGroup.enter()
        self.loadData(isGroupData: true, urlString: "v1/getHotCate", parameters: paratmers) {
            disGroup.leave()
        }
        /// 4.将所有的数据请求完成
        disGroup.notify(queue: DispatchQueue.main) {
            print("所有数据请求完成")
            self.anchorGroups.insert(self.pettyAnchors, at: 0)
            self.anchorGroups.insert(self.bigDataAnchors, at: 0)
            callback()
        }
    }
